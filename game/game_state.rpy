...
#
        ...
        def text_blocks(self, text_to_show):
            block_size = 250
            return [ "\n".join(text_to_show[i:i+block_size]) for i in range(0, len(text_to_show), block_size) ]
        ...
        def timer_running(self):
            return hasattr(self, 'timer') and self.timer.active and self.timer.frames > 0

        def has_random_encounter(self):
            return len(self.map.data()['encounterList']) > 0
        ...
        def show_map(self, in_interaction = False, fade_map = False):
            ...
            renpy.show_screen(
                ...
                has_random_encounter=self.has_random_encounter(),
